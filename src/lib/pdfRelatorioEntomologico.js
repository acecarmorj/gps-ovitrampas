...
/**
 * Gera e baixa o Relatório Entomológico Oficial em PDF (Documento Executivo de 6 Páginas)
 * Exatamente idêntico ao modelo oficial com mapas de calor calibrados de alta definição e satélite Google.
 * @param {Array} armadilhas - Lista de armadilhas registradas
 * @param {Object} opcoes - Filtros e metadados opcionais
 */
export async function gerarRelatorioPdfEntomologico(armadilhas = [], opcoes = {}) {
  if (!armadilhas || armadilhas.length === 0) {
    alert('Nenhuma armadilha registrada para gerar o relatório entomológico.');
    return;
  }
