...
import {
  ...
  ShieldAlert, Info, Layers, RefreshCw, X
} from 'lucide-react';
...
export function CenarioIdealScreen({
  ...
  const pontosIdeais = useMemo(() => getPontosIdeais(), []);
  ...
  const handleCopiarWhatsApp = () => {
    const texto = gerarGuiaWhatsApp(armadilhas, pontosIdeais);
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#F1F2F5] text-slate-900 font-sans select-none overflow-hidden">
      ...
      <header className="bg-white/95 border-b border-slate-200/90 backdrop-blur-md px-3 sm:px-5 py-2.5 shrink-0 shadow-xs z-10">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-2.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-black bg-purple-50 text-purple-700 border border-purple-200 px-1.5 py-0.5 rounded-full flex items-center gap-1">
                  <Sparkles className="w-2.5 h-2.5" />
                  Grade ~300m
                </span>
              </div>
              <p className="text-[10px] text-slate-500 font-medium">
                Planejamento geoespacial otimizado para o próximo ciclo em Carmo-RJ
              </p>
            </div>
          </div>
          ...
          {/* Botões de Ação (WhatsApp & Parecer Técnico IA) */}
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 hover:bg-purple-700 active:scale-95 text-white rounded-xl text-xs font-black transition-all shadow-xs"
              title="Ver Parecer Técnico Oficial para a Gestão / SUS"
            >
              <FileText className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">Parecer Técnico IA</span>
            </button>
          </div>
        ...
        <div className="mt-2 pt-2 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center bg-slate-100 p-0.5 rounded-xl border border-slate-200 text-xs font-black">
            ...
            >
              📋 Lista de Otimização ({pontosFiltrados.length})
            </button>
          </div>
      ...
      {/* 3. MODAL: PARECER TÉCNICO OFICIAL PARA GESTÃO / SUS */}
      {modalParecerAberto && (
